...
export default function Home() {
  ...
  const [loot, setLoot] = useState<{
    label: string;
    type: "xp_boost" | "coins_bonus" | "free_reward";
    value: number | null;
  } | null>(null);
  ...
  const [filter, setFilter] = useState<
    "all" | "daily" | "weekly" | "challenge"
  >("all");
  const [encourageVisible, setEncourageVisible] = useState(false);
  const [encourageMsg, setEncourageMsg] = useState("");
  const [completing, setCompleting] = useState(false);
  const [badgeQueue, setBadgeQueue] = useState<{ key: string; level: number }[]>([]);
  ...
  const fetchHabits = useCallback(
    async (date: string) => {
      const { data, error } = await supabase
        .from("habits")
        .select(
          "id, title, description, xp_reward, frequency, duration_days, is_public, source_habit_id",
        )
      ...
      const sourceIds = habits
        .filter((h) => h.source_habit_id)
        .map((h) => h.source_habit_id as string);

      // Fetch pack info and completion counts for joined challenges
      const joinedHabits = habits.filter((h) => h.source_habit_id);
      const habitsWithDuration = habits.filter(
        (h) => h.source_habit_id && h.duration_days,
      );
      ...
      const [sourcesRes, countsRes] = await Promise.all([
        sourceIds.length > 0
          ? supabase
              .from("habits")
              .select("id, pack:reward_packs(name, difficulty)")
              .in("id", sourceIds)
      ...
      setHabits(
        habits.map((h) => ({
          ...h,
          packInfo: h.source_habit_id
            ? (packMap[h.source_habit_id] ?? null)
            : undefined,
  ...
  useFocusEffect(
    useCallback(() => {
      ...
      fetchStreak();
      supabase
        .rpc("check_badges", { p_user_id: session?.user.id })
        .then(({ data }) => {
  ...
  async function complete(habit: Habit) {
    ...
    if (done.has(habit.id) || completing) return;
    setCompleting(true);
    ...
    const { data: badgeData } = await supabase.rpc("check_badges", {
      p_user_id: session?.user.id,
    });
    const newBadges: string[] = badgeData?.new_badges ?? [];
    if (newBadges.length > 0) {
      const queue = newBadges.map((k) => ({ key: k, level: 0 }));
    ...
    if (data?.loot_label) {
      ...
      setLoot({
        label: data.loot_label,
        type: data.loot_type,
        value: data.loot_value,
      });
      ...
      setEncourageMsg(
        ENCOURAGE_MESSAGES[
          Math.floor(Math.random() * ENCOURAGE_MESSAGES.length)
        ],
      );
  ...
  async function removeHabit(habit: Habit) {
    ...
    if (habit.source_habit_id) {
      ...
      const [{ error: delErr }, { error: leaveErr }] = await Promise.all([
        ...
        supabase
          .from("challenge_leaves")
          .insert({
            user_id: session?.user.id,
            challenge_id: habit.source_habit_id,
          }),
      ]);
  ...
  function renderRightActions(habit: Habit) {
    ...
    return (
      <Pressable style={s.deleteBox} onPress={() => confirmRemove(habit)}>
        <Ionicons
          name={isJoined ? "exit-outline" : "trash"}
          size={20}
          color="white"
        />
  ...
  const filteredHabits = habits.filter((h) => {
    ...
    if (filter === "daily")
      return h.frequency === "daily" && !h.source_habit_id;
    if (filter === "weekly")
      return h.frequency === "weekly" && !h.source_habit_id;
  ...
  return (
    <View style={s.container}>
          ...
          {FILTERS.map((f) => (
            ...
            >
              <Text
                style={[
                  s.filterPillText,
                  filter === f.key && s.filterPillTextActive,
                ]}
              >
                {f.label}
              </Text>
        ...
        <FlatList
          ...
          renderItem={({ item }) => {
            ...
            return (
              <Swipeable
                renderLeftActions={
                  isChallenge ? undefined : () => renderLeftActions(item)
                }
              ...
              >
                <Pressable
                  ...
                  accessibilityRole="button"
                  accessibilityLabel={`${titleClean}, ${isDone ? "complété" : "à faire"}${item.frequency === "daily" ? ", quotidien" : ", hebdomadaire"}`}
                  accessibilityState={{ disabled: isToday && isDone }}
                >
                  {/* Arc gauge */}
                  <View style={s.arcWrap}>
                    <ArcGauge
                      percent={isDone ? 1 : 0}
                      color={
                        isDone ? "#60a5fa" : isChallenge ? "#a78bfa" : "#f9a8d4"
                      }
                      size={62}
                    />
                  ...
                  <View style={s.cardBody}>
                    <Text style={s.cardFreq}>
                      {item.frequency === "daily"
                        ? "Quotidien"
                        : "Hebdomadaire"}
                    </Text>
                    <Text
                      style={[s.cardTitle, isDone && s.cardTitleDone]}
                      numberOfLines={2}
                    >
                      {titleClean}
                    </Text>
                    {item.duration_days && isChallenge ? (
                      <Text style={s.cardProgress}>
                        {item.completionCount ?? 0}/{item.duration_days} jours
                      </Text>
                    ) : null}
                    {item.packInfo ? (
                      <View
                        style={[
                          ...
                          {
                            borderColor:
                              PACK_COLORS[item.packInfo.difficulty] ?? "#888",
                          },
                        ]}
                      >
                        <Ionicons
      ...
      <ConfirmModal
        ...
        onConfirm={() => {
          if (confirmTarget) removeHabit(confirmTarget);
      ...
      <LevelUpModal
        ...
        coinsGained={levelUpModal?.coins ?? 0}
        onClose={() => {
      ...
      <LootModal
        ...
        onGoToShop={() => router.push("/(tabs)/shop")}
        onClose={async () => {
          setLoot(null);
          fetchProfile();
          if (pendingLevelUp) {
      ...
      >
        <Pressable
          style={s.encOverlay}
          onPress={() => setEncourageVisible(false)}
        >
